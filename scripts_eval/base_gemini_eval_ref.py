...
# ============================================================
# ★ GPT 採点関数 → 中身だけ Gemini に変更（引数・形式はそのまま）
# ============================================================
@retry(wait=wait_random_exponential(min=1, max=30),
       stop=stop_after_attempt(10))
def gpt5_eval(pred, input_text, output_text):
...
def run_scoring(input_csv_path, output_csv_path):
    ...
    for idx in tqdm(df.index, desc="Gemini 採点中"):
        ...
        score1, reason1 = gpt5_eval(row["回答1"], row["質問"], row["正解例"])
        df.at[idx, "GPT5評価1"] = score1
        df.at[idx, "GPT5理由1"] = reason1

        score2, reason2 = gpt5_eval(row["回答2"], row["質問"], row["正解例"])
        df.at[idx, "GPT5評価2"] = score2
        df.at[idx, "GPT5理由2"] = reason2

        score3, reason3 = gpt5_eval(row["回答3"], row["質問"], row["正解例"])
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV = PROJECT_DIR / "scripts_eval/gemini_result/base_gemini_ref20.csv"

...
def claude_eval(pred, input_text, output_text, criterion):
    system_prompt = f"""あなたは採点者です。
評価基準をもとに、最終得点と採あなたは採点者です。
...
def run_scoring(input_csv_path, output_csv_path):
    ...
    for idx in tqdm(df.index, desc="Claude 採点中"):
        ...
        score1, reason1 = claude_eval(row["回答1"], row["質問"], row["正解例"], row["評価基準"])
        df.at[idx, "GPT5評価1"] = score1
        df.at[idx, "GPT5理由1"] = reason1

        score2, reason2 = claude_eval(row["回答2"], row["質問"], row["正解例"], row["評価基準"])
        df.at[idx, "GPT5評価2"] = score2
        df.at[idx, "GPT5理由2"] = reason2

        score3, reason3 = claude_eval(row["回答3"], row["質問"], row["正解例"], row["評価基準"])
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV = PROJECT_DIR / "scripts_eval/claude_result/claude_ref_crit_point20.csv"
